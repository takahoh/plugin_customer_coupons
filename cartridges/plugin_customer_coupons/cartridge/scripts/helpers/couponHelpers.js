 ...
 *   （プロパティ nextCouponCode）しか存在しない。挙動は型で異なる:
 *     - SINGLE_CODE  … 常に同一の固定コードを返すだけで消費しない（要 Transaction）。
 *     - MULTIPLE_CODES / SYSTEM_CODES … 呼ぶたびに未発行コードを1つ発行（消費）し
 *       DB ロックを取る。一覧・判定で呼ぶと本番障害につながるため決して呼ばない。
 *   本モジュールでは SINGLE_CODE のときだけ getNextCouponCode() を使う。
 */
...
function getCandidateCoupons() {
    ...
    collections.forEach(plan.getPromotions(), function (promo) {
        ...
        collections.forEach(promo.getCoupons(), function (coupon) {
            ...
            // 実コードを取得できるのは固定コード型 (SINGLE_CODE) のみ。
            ...
            //   （= プロパティ nextCouponCode）しか無い。
            //   - SINGLE_CODE: 常に同一の固定コードを返すだけで消費しない → 安全。
            //     ただし API 仕様上トランザクションが必須。
            //   - MULTIPLE_CODES / SYSTEM_CODES: 呼ぶたびに未発行コードを1つ
            //     発行（消費）し DB ロックを取る。一覧・判定では絶対に呼ばない
            //     （過去にこれで本番障害が発生している）。
            var code = null;
            if (coupon.type === Coupon.TYPE_SINGLE_CODE) {
                try {
                    Transaction.wrap(function () {
                        code = coupon.getNextCouponCode();
                    });
                } catch (e) {
